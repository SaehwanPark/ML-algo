...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    loss = 1 - label * (np.dot(feature_vector, theta) + theta_0)
    if loss<0: loss=0
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    #if hinge_loss_single(feature_vector, label, current_theta, current_theta_0)>=1:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    final_theta = current_theta + 0
    final_theta_0 = current_theta_0 + 0
    ...
    for t in range(T):
        for i in get_order(n):
            ...
            final_theta_0 += current_theta_0
            # if cnt >= 19990:
            #     print(f"Avg Perceptron at {cnt} out of 20000: {current_theta, current_theta_0}")
            pass

    return final_theta / (T * n), final_theta_0 / (T * n)
    raise NotImplementedError
...
def pegasos_single_step_update(
    ...
    if (label * (np.dot(current_theta, feature_vector) + current_theta_0)) <= 1:
        current_theta = (1 - L * eta) * current_theta + eta * feature_vector * label
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(n):
            # Your code here
            update += 1
            eta = 1/np.sqrt(update)
            current_theta, current_theta_0 = pegasos_single_step_update(feature_matrix[i], labels[i], L, eta, current_theta,
                                                                           current_theta_0)
            # if update >= 19990:
            #     print(f"Pegasos at {update} out of 20000: {current_theta, current_theta_0}")
            pass
